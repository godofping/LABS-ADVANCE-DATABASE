...
insert  into `vendorcategories`(`vendorcategoryid`,`vendorcategory`,`isdeleted`) values (1,'Components',0),(2,'Laptops',0),(3,'Smart Phones',0),(4,'Softwares',0),(5,'Peripherals',0),(6,'Hardwares',0),(7,'Uncategories',0);
...
/*!50001 CREATE TABLE  `vendorcategories_view`(
 `Vendor Category ID` int(6) ,
 `Vendor Category` varchar(60) ,
 `isdeleted` tinyint(1) 
)*/;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vendorcategories_view` AS select `vendorcategories`.`vendorcategoryid` AS `Vendor Category ID`,`vendorcategories`.`vendorcategory` AS `Vendor Category`,`vendorcategories`.`isdeleted` AS `isdeleted` from `vendorcategories` where (`vendorcategories`.`isdeleted` = 0) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vendors_view` AS select `vendors`.`vendorid` AS `Vendor ID`,`vendors`.`vendor` AS `Vendor`,`vendorcategories`.`vendorcategory` AS `Vendor Category`,`contactdetails`.`contactnumber` AS `Contact Number`,`contactdetails`.`emailaddress` AS `Email Address`,`addresses`.`address` AS `Address`,`addresses`.`city` AS `City`,`addresses`.`province` AS `Province`,`addresses`.`zipcode` AS `Zip Code`,`contactdetails`.`addressid` AS `addressid`,`vendors`.`contactdetailid` AS `contactdetailid`,`vendors`.`vendorcategoryid` AS `vendorcategoryid`,`vendors`.`isdeleted` AS `isdeleted` from (((`vendors` join `vendorcategories` on((`vendors`.`vendorcategoryid` = `vendorcategories`.`vendorcategoryid`))) join `contactdetails` on((`vendors`.`contactdetailid` = `contactdetails`.`contactdetailid`))) join `addresses` on((`contactdetails`.`addressid` = `addresses`.`addressid`))) */;
