...
CREATE TABLE `orders` (
  ...
  `orderamountpaid` int(6) DEFAULT NULL,
  ...
  CONSTRAINT `FK_o1rders` FOREIGN KEY (`orderamountpaid`) REFERENCES `amounts` (`amountid`),
...
CREATE TABLE `purchaseorders` (
  ...
  `purchaseorderamountpaid` int(6) DEFAULT NULL,
  ...
  CONSTRAINT `FK_purcha23seorders` FOREIGN KEY (`purchaseorderamountpaid`) REFERENCES `amounts` (`amountid`),
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customers_view` AS select `customers`.`customerid` AS `Customer ID`,`basicinformations`.`firstname` AS `First Name`,`basicinformations`.`middlename` AS `Middle Name`,`basicinformations`.`lastname` AS `Last Name`,`genders`.`gender` AS `Gender`,`dates`.`dates` AS `Birth Date`,`contactdetails`.`contactnumber` AS `Contact Number`,`contactdetails`.`emailaddress` AS `Email Address`,`addresses`.`address` AS `Address`,`addresses`.`city` AS `City`,`addresses`.`zipcode` AS `Zip Code`,`addresses`.`province` AS `Province`,`customers`.`contactdetailid` AS `contactdetailid`,`customers`.`basicinformationid` AS `basicinformationid`,`contactdetails`.`addressid` AS `addressid`,`basicinformations`.`genderid` AS `genderid`,`basicinformations`.`birthdate` AS `birthdate` from (((((`customers` join `contactdetails` on((`customers`.`contactdetailid` = `contactdetails`.`contactdetailid`))) join `basicinformations` on((`customers`.`basicinformationid` = `basicinformations`.`basicinformationid`))) join `genders` on((`basicinformations`.`genderid` = `genders`.`genderid`))) join `addresses` on((`contactdetails`.`addressid` = `addresses`.`addressid`))) join `dates` on((`basicinformations`.`birthdate` = `dates`.`dateid`))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staffs_view` AS select `staffs`.`staffid` AS `staffid`,`staffs`.`username` AS `username`,`staffs`.`password` AS `password`,`staffs`.`contactdetailid` AS `contactdetailid`,`staffs`.`basicinformationid` AS `basicinformationid`,`staffs`.`staffpositionid` AS `staffpositionid`,`contactdetails`.`addressid` AS `addressid`,`contactdetails`.`contactnumber` AS `contactnumber`,`contactdetails`.`emailaddress` AS `emailaddress`,`addresses`.`address` AS `address`,`addresses`.`city` AS `city`,`addresses`.`zipcode` AS `zipcode`,`addresses`.`province` AS `province`,`staffpositions`.`staffposition` AS `staffposition`,`basicinformations`.`firstname` AS `firstname`,`basicinformations`.`middlename` AS `middlename`,`basicinformations`.`lastname` AS `lastname`,`basicinformations`.`genderid` AS `genderid`,`basicinformations`.`birthdate` AS `birthdate`,`genders`.`gender` AS `gender` from (((((`staffs` join `contactdetails` on((`staffs`.`contactdetailid` = `contactdetails`.`contactdetailid`))) join `basicinformations` on((`staffs`.`basicinformationid` = `basicinformations`.`basicinformationid`))) join `staffpositions` on((`staffs`.`staffpositionid` = `staffpositions`.`staffpositionid`))) join `genders` on((`basicinformations`.`genderid` = `genders`.`genderid`))) join `addresses` on((`contactdetails`.`addressid` = `addresses`.`addressid`))) */;
