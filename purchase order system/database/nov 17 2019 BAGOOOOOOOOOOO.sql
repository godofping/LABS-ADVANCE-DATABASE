...
CREATE TABLE `orders` (
  ...
  KEY `FK_orders1` (`ordertypeid`),
  KEY `FK_orders2` (`paymentmethodid`),
  KEY `FK_orders3` (`shippingmethodid`),
  CONSTRAINT `FK_orders1` FOREIGN KEY (`ordertypeid`) REFERENCES `ordertypes` (`ordertypeid`),
  CONSTRAINT `FK_orders2` FOREIGN KEY (`paymentmethodid`) REFERENCES `paymentmethods` (`paymentmethodid`),
  CONSTRAINT `FK_orders3` FOREIGN KEY (`shippingmethodid`) REFERENCES `shippingmethods` (`shippingmethodid`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
insert  into `staffs`(`staffid`,`username`,`password`,`contactdetailid`,`basicinformationid`,`staffpositionid`) values (1,'admin','admin',2,1,1);
