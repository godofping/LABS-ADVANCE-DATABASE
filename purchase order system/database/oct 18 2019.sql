...
insert  into `supplierproducts`(`supplierproductid`,`productid`,`supplierid`) values (1,1,1),(2,22,4),(3,77,3),(5,1,3),(6,14,1);
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `paymentmethods_view` AS select `paymentmethods`.`paymentmethodid` AS `paymentmethodid`,`paymentmethods`.`paymentmethod` AS `paymentmethod` from `paymentmethods` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `shippingmethods_view` AS select `shippingmethods`.`shippingmethodid` AS `shippingmethodid`,`shippingmethods`.`shippingmethod` AS `shippingmethod` from `shippingmethods` */;
