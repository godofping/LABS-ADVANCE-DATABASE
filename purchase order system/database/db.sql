...
CREATE TABLE `items` (
  ...
  PRIMARY KEY (`itemid`),
  KEY `FK_items` (`categoryid`),
  CONSTRAINT `FK_items` FOREIGN KEY (`categoryid`) REFERENCES `categories` (`categoryid`)
...
insert  into `users`(`userid`,`username`,`password`) values (1,'admin','admin'),(2,'totoy','totoy');
