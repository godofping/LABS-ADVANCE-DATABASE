...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_products` AS select `products`.`productid` AS `PRODUCT ID`,`products`.`productcategoryid` AS `PRODUCT CATEGORY ID`,`products`.`particularid` AS `PARTICULAR ID`,`products`.`containertypeid` AS `CONTAINER TYPE ID`,`productcategories`.`productcategory` AS `PRODUCT CATEGORY`,`particulars`.`particular` AS `PARTICULAR`,`containertypes`.`containertype` AS `CONTAINER TYPE`,`products`.`price` AS `PRICE` from (((`products` join `productcategories` on((`products`.`productcategoryid` = `productcategories`.`productcategoryid`))) join `particulars` on((`products`.`particularid` = `particulars`.`particularid`))) join `containertypes` on((`products`.`containertypeid` = `containertypes`.`containertypeid`))) */;
