...
insert  into `supplierproducts`(`supplierproductid`,`productid`,`supplierid`) values (1,1,1),(2,22,4);
...
insert  into `suppliers`(`supplierid`,`supplier`,`contactdetailid`,`isdeleted`) values (1,'Supplier 1',18,0),(2,'tae',19,1),(3,'Supplier 2',20,0),(4,'Supplier 3 ',21,0);
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `supplierproducts_view` AS select `supplierproducts`.`supplierproductid` AS `Supplier Product ID`,`suppliers`.`supplier` AS `Supplier`,`products`.`productname` AS `Product Name`,`products`.`productdescription` AS `Product Description`,`products`.`productsku` AS `Product SKU`,`products`.`subcategoryid` AS `subcategoryid`,`products`.`productprice` AS `productprice`,`products`.`isdeleted` AS `productsisdeleted`,`suppliers`.`contactdetailid` AS `contactdetailid`,`supplierproducts`.`productid` AS `productid`,`supplierproducts`.`supplierid` AS `supplierid`,`suppliers`.`isdeleted` AS `suppliersisdeleted` from ((`supplierproducts` join `suppliers` on((`supplierproducts`.`supplierid` = `suppliers`.`supplierid`))) join `products` on((`supplierproducts`.`productid` = `products`.`productid`))) where ((`products`.`isdeleted` = 0) and (`suppliers`.`isdeleted` = 0)) */;
